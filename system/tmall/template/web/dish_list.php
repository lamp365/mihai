				...
				<tr>
					...
					<?php if($item['type'] == 1 && isHasPowerToShow('shop','dish','open_groupbuy')){ ?>
						<?php if($item['open_groupbuy'] == 1){ ?>
							<a  class="btn btn-xs btn-danger" href="<?php  echo web_url('tmall_dish', array('id' => $item['id'], 'op' => 'open_groupbuy','act'=>'close'))?>"><i class="icon-edit"></i>关闭凑单</a>&nbsp;&nbsp;
						<?php }else{ ?>
							<a  class="btn btn-xs btn-warning" href="<?php  echo web_url('tmall_dish', array('id' => $item['id'], 'op' => 'open_groupbuy','act'=>'open'))?>"><i class="icon-edit"></i>开启凑单</a>&nbsp;&nbsp;
						<?php } ?>
					<?php } ?>
					<?php if(isHasPowerToShow('shop','dish','post','edit')){ ?>
						<a  class="btn btn-xs btn-info" href="<?php  echo web_url('tmall_dish', array('id' => $item['id'], 'op' => 'post'))?>"><i class="icon-edit"></i>&nbsp;编&nbsp;辑&nbsp;</a>&nbsp;&nbsp;
					<?php } ?>
					<?php if(isHasPowerToShow('shop','dish','delete')){ ?>
						<a  class="btn btn-xs btn-info" href="<?php  echo web_url('tmall_dish', array('id' => $item['id'], 'op' => 'delete'))?>" onclick="return confirm('此操作不可恢复，确认删除？');return false;"><i class="icon-edit"></i>&nbsp;删&nbsp;除&nbsp;</a>
					<?php } ?>
						&nbsp;&nbsp;					
					</td>
				</tr>
