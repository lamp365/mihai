<?php
        ...
        if ($operation == 'post') {
            ...
            $tmall_shop = mysqld_selectall("SELECT * FROM ".table('tmall'));
            $id = intval($_GP['id']);
            if (!empty($id)) {
				$item = mysqld_select("SELECT a.*,b.title as gname,b.id as gid,b.thumb as gthumb FROM " . table('tmall_dish') . " AS a LEFT JOIN " . table('shop_goods') . " as b on a.gid = b.id WHERE a.id = :id", array(':id' => $id));
                if (empty($item)) {
...
//                    'timeend' => strtotime($_GP['timeend']),
                    ...
                    'tmall_id' => intval($_GP['tmall_shop']),
                    );
                ...
                if (empty($id)) {
                    ...
                    unset($data['createtime']);
                     unset($data['sales']);
                    mysqld_update('tmall_dish', $data, array('id' => $id));
                }
				$vip_data = array();
  ...
  $tspecitems = mysqld_select("SELECT id FROM " . table('shop_dish_spec_item') . " WHERE id = :id", array(':id' => $get_item_id));
                ...
                //总库存
                if ($totalstocks > 0) {
                    mysqld_update("tmall_dish", array("total" => $totalstocks), array("id" => $id));
                }
			...
			// 找出所有需要更新数据的信息
            $dish_error = mysqld_selectall("SELECT * FROM ".table('tmall_dish'). " WHERE title = '' or marketprice <= 0 or timeprice <= 0 or productprice <= 0");
            if ( $dish_error ) {
            // 执行更新操作
                foreach ( $dish_error as $dish_date ){
					...
					if ($shop_goods){
							...
							mysqld_update('tmall_dish', $data, array('id' => $dish_date['id']));   
					}
				}
			}
           ...
           if (!empty($_GP['report'])) {
            ...
            // $list = mysqld_selectall("SELECT a.* FROM " . table('tmall_dish') . " as a WHERE  a.deleted=0 $condition ORDER BY a.status DESC, a.displayorder DESC, a.id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
			//echo "SELECT * FROM " . table('tmall_dish') . " WHERE  deleted=0 $condition ORDER BY status DESC, displayorder DESC, id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
			foreach($list as $key=>$val){
             ...
             $total = mysqld_selectcolumn('SELECT COUNT(*) FROM ' . table('tmall_dish') . " as a WHERE $condition");
             $pager = pagination($total, $pindex, $psize);
             include page('dish_list');
        } elseif ($operation == 'delete') {
            $id = intval($_GP['id']);
            $row = mysqld_select("SELECT id, thumb FROM " . table('tmall_dish') . " WHERE id = :id", array(':id' => $id));
            if (empty($row)) {
                message('抱歉，商品不存在或是已经被删除！');
            }
            mysqld_delete("tmall_dish", array('id' => $id));
            if(mysqld_insertid()){
