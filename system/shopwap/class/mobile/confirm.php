...
if (!empty($id) && $_GP['op'] != 'cart' ) {
	...
	$shipname = mysqld_select("select name from ".table('dish_list')." where id =:shipprice ",array(':shipprice'=>$item['transport_id']));
	...
	$direct = true;
	$returnurl = mobile_url("confirm", array("id" => $id, "spec_key" => $spec_key, "total" => $total));
}
if (!$direct) {
	//如果不是直接购买（从购物车购买）
	...
	if (!empty($list)) {
		...
		foreach ($list as &$g) {
			...
			$taxprice = 0;
			//获取规格价格 和库存
			$item = get_spec_price($g['spec_key'],$item);
...
if(empty($issendfree)){
	...
	if($totalprice >= $promotion['condition']){
		$issendfree=1;
	}
}
$dispatchprice = 0;
