<?php
	...
	if($operation=='post')
	{
		...
		if($bonus['send_type']==1){
			   ...
			   $bonus_good_list = mysqld_selectall("SELECT bonus_good.*,shop_goods.title FROM " . table('bonus_good') . " bonus_good left join " . table('shop_dish') . " shop_goods on shop_goods.id=bonus_good.good_id WHERE  bonus_good.bonus_type_id=:bonus_type_id",array(":bonus_type_id"=>intval($_GP['id'])));
			   $goodslist = mysqld_selectall("SELECT * FROM " . table('shop_dish') . " WHERE  deleted=0 $condition and status=1 and id not in (SELECT good_id FROM " . table('bonus_good') . " WHERE  bonus_type_id=:bonus_type_id)",array(":bonus_type_id"=>intval($_GP['id'])));
			   if (checksubmit('send_goods')) {
					 ...
					 foreach ($_GP['good_add'] as &$goodid) {
					  $bonus_good = mysqld_select("SELECT * FROM " . table('bonus_good') . " WHERE  bonus_type_id=:bonus_type_id and good_id=:good_id",array(":bonus_type_id"=>intval($_GP['id']),":good_id"=>intval($goodid)));
							...
							$goods = mysqld_select("SELECT * FROM " . table('shop_dish') . " WHERE  deleted=0 and id=:goodid and status=1",array(":goodid"=>$goodid));
		...
		$goodslist = mysqld_selectall("SELECT id,title FROM " . table('shop_dish') . " {$condition} limit 25");
		if(empty($goodslist)){
			die(showAjaxMess(1002,'暂时没有数据！'));
		}else{
			die(showAjaxMess(200,$goodslist));
		}
	}