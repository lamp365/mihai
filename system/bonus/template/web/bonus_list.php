...
<h3 class="header smaller lighter blue">优惠券管理&nbsp;&nbsp;&nbsp;
    ...
    <select name="" id="choose_bonus" style="font-size: 14px;padding: 6px 2px;">
        <option value="-1">全部优惠卷</option>
        <?php foreach($bonus_enum_arr as $key_type => $bon) {
            $sel = '';
            if(empty( $_GP['type']) &&  $_GP['type'] == null){
                $sel = '';
            }else if($_GP['type'] == $key_type){
                $sel = "selected";
            }
    ...
    <th class="text-center">操作</th>
		...
		<?php  if(is_array($bonus_list)) { foreach($bonus_list as $item) { ?>
          ...
          <td class="text-center">
          	<?php echo $bonus_enum_arr[$item['send_type']]; ?>
			</td>
...
$("#choose_bonus").change(function(){
    var type = $(this).val();
    var url = "<?php echo web_url('bonus',array('name'=>'bonus','op'=>'display'));?>";
    url = url +"&type="+type;
