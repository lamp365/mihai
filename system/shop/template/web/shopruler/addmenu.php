<div class="alertModal-dialog-bg" style="width:40%">
    <form action="<?php echo web_url('shopruler',array('op'=>'do_addmenu'))?>" method="post" class="form-horizontal"  >
        ...
        <div class="modal-body">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-4">
                    <?php if(empty($_GP['rule_id'])){ ?>
                    <select name="pid" id="" class="form-control">
                        <option value="0">顶级菜单</option>
                        <?php foreach($menu as $row){ ?>
                            <option value="<?php echo $row['rule_id'];?>"  <?php if($row['rule_id'] == $_GP['pid']){ echo "selected";} ?>><?php echo $row['rule_name'];?></option>
                        <?php } ?>
                    </select>
                    <?php }else{
                        echo ' <input type="hidden" value="0" class="col-xs-10 col-sm-4 form-control" name="pid">';
                        echo ' <input type="text" value="顶级菜单" class="col-xs-10 col-sm-4 form-control" disabled>';
                    } ?>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-4">
                    <input type="text"  name="moddo" class="col-xs-10 col-sm-4 form-control"  value="<?php if(!empty($editMenu)){echo $editMenu['moddo'];} ?>"/>
                </div>
            </div>
