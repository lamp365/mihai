...
<form action="" method="post" name="sqlform" enctype="multipart/form-data" class="form-horizontal" onsubmit="return toVaild()">
	...
	<input type="hidden" name="parentid" value="<?php  echo $parent['id'];?>" />
	   ...
	   <div class="form-group">
    	...
    	<?php  if(!empty($category['industry_p2_id']) | $parentid ) { ?>
            <div class="form-group">
                <label class="col-sm-2 control-label no-padding-left" >所在行业</label>
                    <div class="col-sm-9">
                        <?php  echo $category['industry_p1_name']."-".$category['industry_p2_name'];?>
                        <input type="hidden" name="industry_p1_id" value=" <?php  echo $category['industry_p1_id'];?>">
                        <input type="hidden" name="industry_p2_id" value=" <?php  echo $category['industry_p2_id'];?>">
                    </div>
            </div>
        <?php  }else{     ?>
        <div class="form-group">
          <label class="col-sm-2 control-label no-padding-left" >所在行业</label>
          <div class="col-sm-9">
              <select  id="cate_1" style="margin-right:15px;" name="industry_p1_id" class="pcates" onchange="fetchChildCategory(this, this.options[this.selectedIndex].value)"  autocomplete="off">
                  <option value="0">请选择一级行业</option>
                  <?php if (is_array($industry_data)) {
                      foreach ($industry_data as $row) {  ?>
                          <option value="<?php echo $row['gc_id']; ?>" <?php if ($row['gc_id'] == $category['industry_p1_id']) { ?> selected="selected"<?php } ?>><?php echo $row['gc_name']; ?></option>
                      <?php }       }   ?>
              </select>
              <select  id="cate_2" name="industry_p2_id" class="cate_2"   autocomplete="off">
...
<script>
    ...
    <script language="javascript">
        var category = <?php echo json_encode($industry_data) ?>;
        function fetchChildCategory(o_obj, cid) {
            var html = '<option value="0">请选择二级分类</option>';
            if (!category || !category[cid]['sub']) {
            }else{
                var subdata= category[cid]['sub'];
                for (i in subdata) { 
//                    console.log(subdata[i]);
                    html += '<option value="' + subdata[i]['gc_id'] + '">' + subdata[i]['gc_name'] + '</option>';
                }
            }
            $("#cate_2").html(html);
        }
