...
<body class="J_scroll_fixed">
        ...
        <div class="well form-search">
            ...
            <form action="" method="post" class="form-horizontal" enctype="multipart/form-data" >
                <div class="mb10">
                        ...
                        <br/><br/>
                        <label class="col-sm-2 control-label no-padding-left">所属分类：</label>
                        <select  style="margin-right:15px;" id="p1" name="p1"  autocomplete="off" onchange="get_son_cat(this,2)">
                            <option value="0">请选择分类</option>
                            <?php foreach($all_category as $item) {
                                if($item['id'] == $this_brand['p1']){
                                    $sel = "selected";
                                }else{
                                    $sel = '';
                                }
                        ...
                        <select  style="margin-right:15px;" id="p2" name="p2"  autocomplete="off" onchange="get_son_cat(this,3)">
                            <option value="0">请选择分类</option>
                            <?php foreach($first_son as $item2) {
                                if($item2['id'] == $this_brand['p2']){
                                    $sel = "selected";
                                }else{
                                    $sel = '';
                                }
                        ...
                        <select  style="margin-right:15px;" id="p3" name="p3"  autocomplete="off" >
                            <option value="0">请选择分类</option>
                            <?php foreach($second_son as $item3) {
                                if($item3['id'] == $this_brand['p3']){
                                    $sel = "selected";
                                }else{
                                    $sel = '';
                                }
                                echo "<option value='{$item3['id']}' {$sel}>{$item3['name']}</option>";
                            }
                            ?>
                        </select>
                        <br/><br/>
    ...
    <script>
        function get_son_cat(obj,num)
        {
            var id = $(obj).val();
            var url ="<?php echo web_url('brand',array('op'=>'add','ajaxCat'=>1,'name'=>'shop')); ?>";
            if(id == 0){
                ...
                $.post(url,{'id':id},function(data){
                    if(data.errno == 200){
                        ...
                        if(num == 2){
                            $("#p2").html(html);
                            $("#p3").html("<option value='0'>请选择分类</option>");
                        }else if(num == 3){
                            $("#p3").html(html);
                        }
                    }else{
                        if(num == 2){
                            $("#p2").html("<option value='0'>请选择分类</option>");
                            $("#p3").html("<option value='0'>请选择分类</option>");
                        }if(num == 3){
                            $("#p3").html("<option value='0'>请选择分类</option>");
                        }
