...
，
	  ...
	  <div class="form-group">
	  	<label class="col-sm-2 control-label no-padding-left" > 配送方式：</label>
						<div class="col-sm-9">
                   	            <?php
                                           if ( is_array($payment)){
											      $item_way  =  unserialize($item['payway']);
												  if ( !is_array($item_way) ){
                                                         $item_way = array();
												  }
                                                  foreach ( $payment as $value ){
													     $check = '';
													     if ( in_array($value['id'], $item_way )){
                                                                $check = 'checked';
														 }
                                                         echo '<input '.$check.' type="checkbox" name="payway[]" value="'.$value['id'].'" />&nbsp;<label>'.$value['name'].'</label>&nbsp;';
												  }
										}
								?>
						</div>
