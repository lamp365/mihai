...
<h3 class="header smaller lighter blue"><?php  if(!empty($item['id'])) { ?>编辑<?php  }else{ ?>新增<?php  } ?>换购
</h3>
...
<form role="form" class="form-horizontal">
 ...
 <div class="form-group">
		...
		<div class="col-sm-9">
			  <select name="c_goods" id="c_goods" style="width: 50%">
 ...
 <div class="form-group" style="display:none;">
		<label class="col-sm-2 control-label no-padding-left" > 换购名称：</label>

		<div class="col-sm-9">
					 <input type="text" name="messname" id="messname" maxlength="100" class="span7"  value="<?php  echo $item['title'];?>" />
		</div>
</div>
 <div class="form-group" style="display:none">
										<label class="col-sm-2 control-label no-padding-left" > 编号：</label>

										<div class="col-sm-9">
												 <input type="text" name="messsn"  value="<?php  echo $item['messsn'];?>" /> 
										</div>
		</div>

 <div class="form-group">
										<label class="col-sm-2 control-label no-padding-left" > 是否开通换购：</label>
		 ...
		 <div class="form-group">
                ...
                <option value="0">请选择一级价格区间</option>
            ...
            <select  id="cate_2" name="ccate" autocomplete="off">
                <option value="0">请选择二级价格区间</option>
		...
		<div class="form-group" >
										<label class="col-sm-2 control-label no-padding-left" > 换购价格：</label>
			...
			<div class="form-group" style="display:none">
										<label class="col-sm-2 control-label no-padding-left" > 订单价格：</label>
				...
				<div class="form-group">
										<label class="col-sm-2 control-label no-padding-left" > 限购数量(0为不限购)：</label>

										<div class="col-sm-9">
												<input type="text" name="max_buy" id='max_buy' value="<?php  echo empty($item['max_buy'])?'0':$item['max_buy'];?>" />件
										</div>
		</div>
          ...
          <input type="radio" name="totalcnf" value="1" id="totalcnf2"  <?php  if(!empty($item) && $item['totalcnf'] == 1) { ?>checked="true"<?php  } ?> /> 永不减库存
				...
				<div class="form-group">
										<label class="col-sm-2 control-label no-padding-left" > 换购属性：</label>
				 ...
				 <input type="checkbox" name="isrecommand" value="1" id="isrecommand" <?php  if($item['isrecommand'] == 1) { ?>checked="true"<?php  } ?> /> 首页推荐
					...
					<div class="form-group" style="display:none">
										<label class="col-sm-2 control-label no-padding-left" > 免运费换购：</label>

										<div class="col-sm-9">
				 <input type="checkbox" name="issendfree" value="1" id="isnew" <?php  if($item['issendfree'] == 1) { ?>checked="true"<?php  } ?> /> 打勾表示此换购不会产生运费花销，否则按照正常运费计算。
           &nbsp;   
										</div>
		</div>
		
			<div class="form-group" style="display:none">
										<label class="col-sm-2 control-label no-padding-left" > 限时换购：</label>

										<div class="col-sm-9">
				 
<input type="checkbox" name="istime" id='istime' value="1" id="isnew" <?php  if($item['istime'] == 1) { ?>checked="true"<?php  } ?> /> 开启限时换购
	...
	<script type="text/javascript">
		$("#datepicker_timeend").datetimepicker({
            ...
            <p class="help-block">会员购买换购赠送的积分, 如果不填写，则默认为不奖励积分</p>
				...
				<div class="form-group" style="display:none;">
					<label class="col-sm-2 control-label no-padding-left" >换购主图：<br/>（建议统一640*640 或640*320; 默认为产品图片）</label>
										<div class="col-sm-9">
				...
				<div class="form-group">
										<label class="col-sm-2 control-label no-padding-left" >换购简单描述：</label>
				...
				<div class="form-group" style="display:none">
										<label class="col-sm-2 control-label no-padding-left" >换购详细描述：<br/><span style="font-size:12px">(建议图片宽不超过640px;如果为空则调用产品的详情图)</span></label>
...
<?php  include page('mess_option');?>
      
      	<div class="form-group">
										<label class="col-sm-2 control-label no-padding-left" ></label>

										<div class="col-sm-9">
				    <button type="submit" class="btn btn-primary span2" name="submit" value="submit"><i class="icon-edit"></i>保存换购信息</button>    
										</div>
		</div>
...
<script language="javascript">
   ...
   function fetchChildarea(cid) {
	var html = '<option value="0">请选择二级价格区间</option>';
	if (!area || !area[cid]) {
		$('#cate_2').html(html);
		return false;
	}
