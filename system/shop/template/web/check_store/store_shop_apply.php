		...
		<?php if(is_array($list)) { foreach($list as $item) { ?>
				<tr>
                    ...
                    <td style="text-align:center;">
						<?php echo $storeType[$item['sts_shop_type']];  ?>
						<br/>
					...
					<td style="text-align:center;">
                        <span id='span_<?php  echo $item['sts_id'];?>'  class="label 
                          ...
                          <?php  if($item['sts_info_status']==1 || $item['sts_info_status']==12){echo "label-danger";}?>
                              " style="cursor:pointer;"><?php  echo $this->info_status_text[$item['sts_info_status']];?></span>
					</td>
					<td style="text-align:center;">
						<a  class="btn btn-xs btn-info" data-url="<?php  echo web_url('store_shop_manage', array('id' => $item['sts_id'], 'op' => 'shenhe', 'sts_info_status' => '0'))?>" onclick="shenhePass(this,<?php echo $item['sts_id']?>);"><i class="icon-edit"></i>审核通过</a>&nbsp;&nbsp;
						<a  class="btn btn-xs btn-info" data-url="<?php  echo web_url('store_shop_manage', array('id' => $item['sts_id'], 'op' => 'dialog'))?>" onclick="openDialog(this);"><i class="icon-edit"></i>审核不通过</a>
					</td>
				</tr>
				<?php  } } ?>
...
//审核通过ajax
 function shenhePass(obj,id){
    var url = '<?php echo web_url('store_shop_manage',array('op'=>'dialog2'))?>';
     $.ajaxLoad(url,{id:id},function(ret){
            $('#alterModal').modal('show');
    });
    var url = $(obj).data('url');
}
