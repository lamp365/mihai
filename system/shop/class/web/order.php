...
if ($operation == 'display') {
	...
	if(!empty($_GP['dishid'])){
		$condition .= " AND B.goodsid={$_GP['dishid']}";
	}
	...
	if($status == -2){ //退款
		...
		$sqlNum = "SELECT COUNT(distinct A.id) FROM " . table('shop_order') . " A  left join ". table('shop_order_goods')." as B on A.id=B.orderid WHERE  {$condition} and B.type=3 and B.status in (1,2)";
	}else if($status == -4){  //退货
		...
		$sqlNum = "SELECT COUNT(distinct A.id) FROM " . table('shop_order') . " A  left join ". table('shop_order_goods')." as B on A.id=B.orderid WHERE  {$condition} and B.type=1 and B.status in (1,2,3)";
		...
		if(!empty($_GP['dishid'])){
			//按照宝贝id搜索
			$sql    = "select A.* from ".table('shop_order')." A left join ".table('shop_order_goods')." B on A.id=B.orderid where {$condition} ORDER BY  A.createtime DESC ".$selectCondition;
			$sqlNum = 'SELECT COUNT(A.id) FROM ' . table('shop_order') . " A left join ".table('shop_order_goods')." B on A.id=B.orderid WHERE  {$condition}";
		}else{
...
elseif($operation == 'identity')
{
	...
	if ($order['relation_uid'] == '0' OR $order['relation_uid'] == 0) {
		//身份证信息
		...
		// 一件代发身份证信息
		$identity =  mysqld_select("SELECT idname as identity_name,idnumber as identity_number FROM ".table('shop_address')." WHERE id=:addressid", array(':addressid'=>$order['addressid']));
	}
