<?php
         ...
         if ($operation == 'display') {
               if(!empty($_GP['type'])){
                   $where = "where type={$_GP['type']}";
               }
               if(!empty($_GP['showpage'])){
                   $where = "where page={$_GP['showpage']}";
               }
                $list = mysqld_selectall("SELECT * FROM " . table('shop_adv') . " {$where} ORDER BY displayorder DESC");
            ...
            if (checksubmit('submit')) {
                $data = array(
                	...
                	'page' =>intval($_GP['page']),
                	'position' =>intval($_GP['position']),
