...
$list = mysqld_selectall("SELECT goods.*,0 as cpersent,(select sum((ordergoods.price*ordergoods.total)) from  " . table('shop_order_goods') . " ordergoods where ordergoods.shopgoodsid=goods.id $condition) salesmoney,(select sum(ordergoods.total) from  " . table('shop_order_goods') . " ordergoods where ordergoods.shopgoodsid=goods.id $condition) salescount  from " . table('shop_goods') . " goods order by salesmoney  desc");
file_put_contents('xxx.txt', serialize($list));
