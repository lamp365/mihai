...
<div class="panel with-nav-tabs panel-default" style="margin-top: 20px">	
	    <div class="panel-heading">
	            <ul class="nav nav-tabs">
	                <li <?php if ($request_type==1) {echo 'class="active"';}?> ><a href="<?php echo web_url('applyed',array('id'=>$_GP['id'],'request_type'=>1));?>" >心愿记录</a></li>
					<li <?php if ($request_type==2) {echo 'class="active"';}?> ><a href="<?php echo web_url('applyed',array('id'=>$_GP['id'],'request_type'=>2));?>" >兑换记录</a></li>
	            </ul>
	    </div>
	    ...
	    <?php }else{ ?>
            <div class="tab-pane fade in active" >
				...
				<li style="float:left;list-style-type:none;margin-bottom: 15px;">
					<select name="state" onchange="sel_by_status(this)" style="margin-right:10px;margin-top:10px;width: 100px; height:34px; line-height:28px; padding:2px 0">
						<option value="0" <?php if($_GP['jifen_status'] ==0){ echo "selected";}?>>全部记录</option>
						<option value="2" <?php if($_GP['jifen_status'] ==2){ echo "selected";}?>>兑换成功</option>
						<option value="3" <?php if($_GP['jifen_status'] ==3){ echo "selected";}?>>兑换失败</option>
						<option value="1" <?php if($_GP['jifen_status'] ==1){ echo "selected";}?>>正在申请</option>
					</select>
					&nbsp;
				...
				<br/>
						<table class="table table-striped table-bordered table-hover">
							...
							<tbody>
								...
								<tr>
									<td style="text-align:center;">
										<?php if($item['jifen_status'] != 1){ $dis="disabled";}else{ $dis = '';} ?>
									...
									<td style="text-align:center;">
										...
										<?php }else if($item['jifen_status'] == 2){ ?>
											<span class="btn btn-xs btn-success">兑换成功</span>
										<?php }else if($item['jifen_status'] == 3){?>
			...
			<script>
				function sel_by_status(obj){
					var jifen_status = $(obj).val();
					var url = window.location.href;
					url = url+"&jifen_status="+jifen_status;
