...
`system` TINYINT(1)  DEFAULT '1' COMMENT '类型 1安卓 2ios 3平板 4pc',
`count_type` TINYINT(1)  DEFAULT '1' COMMENT '类型 1商城页面访问 2登录 3注册  4下载app',
...
function trafficCount(){
    ...
    //获取访问的商品id
    $shop_id = 0;
    if($_GET['do'] == 'groupbuy' && $_GET['op']=='detail_group'){
        $insert_mark = 'shop';
        $shop_id = $_GET['id'];
    }else if($_GET['do'] == 'detail' && $_GET['op']=='dish'){
        $insert_mark = 'shop';
        $shop_id = $_GET['id'];
    }
