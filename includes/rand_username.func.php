...
function get_rand_username(){
    ...
    $tou_num = array_rand($nicheng_tou);

    $wei_num = array_rand($nicheng_wei);
...
function get_rand_mobile($trueNum = ''){
    $arr = array(
    ...
    //真实号码11位，避免做虚拟用户，该用户占用了真实的手机号码，则虚假号码可以设置成10位
    if($trueNum){
        //真实号码11位
        $mobile = $arr[array_rand($arr)].mt_rand(1000,9999).mt_rand(1000,9999);
    }else{
        //虚假号码10位
        $mobile = $arr[array_rand($arr)].mt_rand(100,999).mt_rand(1000,9999);
    }
