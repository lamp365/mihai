...
class shopindexService extends publicService
{
    ...
    public function getStoreInfoAndgoods($data = array()){
        ...
        $storeIcon = $storeShopModel->getAll('','sts_id,sts_name,sts_avatar,sts_summary',"sts_score DESC LIMIT {$limit} , {$psize}");
        if ($storeIcon){
            foreach ($storeIcon as $k=>$v){
                ...
                $info = $shopDishModel->getAll(to_sqls($where),"id,title,thumb,marketprice,productprice","id DESC LIMIT 0 , 2");
    ...
    public function getStoreInfoArr(){
        ...
        $storeList = $storeShopModel->getAll('','sts_id,sts_name,sts_avatar','sts_score desc');
