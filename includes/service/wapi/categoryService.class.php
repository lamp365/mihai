...
class categoryService extends \service\publicService
{
     ...
     * 取参加活动的行业名称
     *   */
    public function getActInsName($ac_id){
        if (empty($ac_id)) return '';
        $actDishModel = new \model\activity_dish_model();
     ...
     * 取参加活动的一级栏目名称
     *   */
    public function getP1CatName($ac_id){
        if (empty($ac_id)) return '';
        $actDishModel = new \model\activity_dish_model();
	    $table1 = $actDishModel->table_name;
     ...
     *   */
    public function getCat1NameByActIns($ins_id){
        if (empty($ins_id)) return '';
        $actDishModel = new \model\activity_dish_model();
	    $table1 = $actDishModel->table_name;
	    $table2 = 'shop_category';
	    $where = " a.ac_in_id = '$ins_id' and a.ac_dish_status=1 ";
     ...
     *   */
    public function getCat2NameByActP1Cat($p1_id){
        if (empty($p1_id)) return '';
        $actDishModel = new \model\activity_dish_model();
	    $table1 = $actDishModel->table_name;
	    $table2 = 'shop_category';
	    $where = " a.ac_p1_id = '$p1_id' and a.ac_dish_status=1 ";
