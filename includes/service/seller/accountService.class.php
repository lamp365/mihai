...
class accountService extends \service\publicService
{
    ...
    public function dashbord_data_info($sts_id)
    {
        ...
        $today_order   = mysqld_select("SELECT count(id) as order_num, sum(price-store_earn_price-plate_money-return_money) as price FROM ".table('shop_order')." WHERE {$where} and status >= 1 and paytime >=".$today_zero);
        $today_price   = FormatMoney($today_order['price'],0);   //钱转为元 页面显示一定要用元

        //昨天的订单金额 订单量  已经下单的
        $yestoday_zero  = strtotime(date("Y-m-d",strtotime("-1 day")));   //昨天的凌晨时间点
        $yestoday_order = mysqld_select("SELECT count(id) as order_num, sum(price-store_earn_price-plate_money-return_money) as price FROM ".table('shop_order')." WHERE {$where} and status >= 1 and paytime >=".$yestoday_zero." and paytime<=".$today_zero);
    ...
    public function dashborder_money_line($sts_id=0,$time='week'){
        ...
        foreach($time_arr as $time){
            $s_time = $time;
            $e_time = $time+3600*24;
            $price_total = mysqld_select("SELECT sum(price-store_earn_price-plate_money-return_money) as price FROM ".table('shop_order')." WHERE {$where} and status >= 1 and paytime >=".$s_time." and paytime<=".$e_time);
