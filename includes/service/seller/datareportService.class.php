...
class datareportService extends \service\publicService
{
    ...
    public function wait_income($sts_id)
    {
        ...
        $wait_order = mysqld_select("SELECT count(id) as order_num, sum(price-store_earn_price-plate_money-return_money) as price FROM ".table('shop_order')." WHERE sts_id={$sts_id} and (status = 1 or status =2)");
    ...
    public function sales_data_info($sts_id,$type,$data)
    {
        ...
        //获取收益 与已经支付订单量
        $pay_info = mysqld_select("SELECT count(id) as order_num, sum(price-store_earn_price-plate_money-return_money) as price FROM ".table('shop_order')." WHERE sts_id={$sts_id} and status >= 1 and paytime >=".$s_time." and paytime<=".$e_time);
        $money    = FormatMoney($pay_info['price'],0);
        $pay_num  = $pay_info['order_num'];

        //获取收益 与等待支付订单量
        $wait_info  = mysqld_select("SELECT count(id) as order_num, sum(price-store_earn_price-plate_money-return_money) as price FROM ".table('shop_order')." WHERE sts_id={$sts_id} and status = 0 and paytime >=".$s_time." and paytime<=".$e_time);
