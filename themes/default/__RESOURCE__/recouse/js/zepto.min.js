...
var Zepto = function() {
    ...
    f = e.slice,
    g = e.filter,
    h = window.document?window.document:window.body,
    i = {},
    j = {},
...
function(a) {
    function b(l) {
        ...
        h = !g && l.match(/(iPhone\sOS)\s([\d_]+)/),
        j = l.match(/(webOS|hpwOS)[\s\/]([\d.]+)/),
        m = j && l.match(/TouchPad/),
        n = l.match(/Kindle\/([\d.]+)/),
        q = l.match(/Silk\/([\d._]+)/),
        G = l.match(/(BlackBerry).*Version\/([\d.]+)/),
        d = l.match(/(BB10).*Version\/([\d.]+)/),
        p = l.match(/(RIM\sTablet\sOS)\s([\d.]+)/),
        k = l.match(/PlayBook/),
        i = l.match(/Chrome\/([\d.]+)/) || l.match(/CriOS\/([\d.]+)/),
        c = l.match(/Firefox\/([\d.]+)/);
        if (o.webkit = !!H) {
            o.version = H[1]
        }
        f && (e.android = !0, e.version = f[2]),
        h && (e.ios = e.iphone = !0, e.version = h[2].replace(/_/g, ".")),
        g && (e.ios = e.ipad = !0, e.version = g[2].replace(/_/g, ".")),
        j && (e.webos = !0, e.version = j[2]),
        m && (e.touchpad = !0),
        ...
        q && (o.silk = !0, o.version = q[1]),
        !q && e.android && l.match(/Kindle Fire/) && (o.silk = !0),
        i && (o.chrome = !0, o.version = i[1]),
        c && (o.firefox = !0, o.version = c[1]),
        e.tablet = !!(g || k || f && !l.match(/Mobile/) || c && l.match(/Tablet/)),
        e.phone = !e.tablet && !!(f || h || j || G || d || i && l.match(/Android/) || i && l.match(/CriOS\/([\d.]+)/) || c && l.match(/Mobile/))
    }
...
function(a) {
    ...
    function u(w, x, y, z) {
        ...
        return (c[q(w)] || []).filter(function(A) {
            return A && (!x.e || A.e == x.e) && (!x.ns || v.test(A.ns)) && (!y || q(A.fn) === q(y)) && (!z || A.sel == z)
        })
    }
    function g(v) {
        var w = ("" + v).split(".");
    ...
    function o(w, x, v) {
        a.type(w) != "string" ? a.each(w, v) : w.split(/\s/).forEach(function(y) {
            v(y, x)
        })
    }
    function h(v, w) {
    ...
    function p(B, z, A, v, w, x) {
        ...
        function(G, E) {
            ...
            F.fn = E,
            F.sel = v,
            F.e in j && (E = function(I) {
                var H = I.relatedTarget;
                if (!H || H !== this && !a.contains(this, H)) {
            ...
            var D = F.del || E;
            F.proxy = function(I) {
                var H = D.apply(B, [I].concat(I.data));
                return H === !1 && (I.preventDefault(), I.stopPropagation()),
                H
            },
            F.i = C.length,
            C.push(F),
            B.addEventListener(l(F.e), F.proxy, h(F, x))
        })
    }
    function r(x, z, y, v, A) {
        var w = q(x);
        o(z || "", y,
        function(B, C) {
            u(x, B, C, v).forEach(function(D) {
                delete c[w][D.i],
                x.removeEventListener(l(D.e), D.proxy, h(D, A))
            })
        })
    }
    function i(w) {
        ...
        for (x in w) { ! n.test(x) && w[x] !== undefined && (v[x] = w[x])
        }
        return a.each(s,
        function(y, z) {
            v[y] = function() {
                return this[z] = t,
                w[y].apply(w, arguments)
            },
            v[z] = f
        }),
        v
    ...
    function m(v) {
        if (! ("defaultPrevented" in v)) {
            v.defaultPrevented = !1;
            var w = v.preventDefault;
    ...
    a.proxy = function(w, x) {
        ...
        if (typeof x == "string") {
            return a.proxy(w[x], w)
        }
        throw new TypeError("expected function")
    },
    a.fn.bind = function(v, w) {
        return this.each(function() {
            p(this, v, w)
        })
    },
    a.fn.unbind = function(v, w) {
        return this.each(function() {
            r(this, v, w)
        })
    },
    a.fn.one = function(v, w) {
        return this.each(function(x, y) {
            p(this, v, w, null,
            function(z, A) {
                return function() {
                    var B = z.apply(y, arguments);
                    return r(y, A, z),
                    B
                }
            })
    ...
    a.fn.delegate = function(w, x, v) {
        return this.each(function(z, y) {
            p(y, x, v, w,
            function(A) {
                return function(B) {
                    var C, D = a(B.target).closest(w, y).get(0);
                    if (D) {
                        return C = a.extend(i(B), {
                            currentTarget: D,
                            liveFired: y
                        }),
                        A.apply(D, [C].concat([].slice.call(arguments, 1)))
                    }
                }
            })
        })
    },
    a.fn.undelegate = function(x, v, w) {
        return this.each(function() {
            r(this, v, w, x)
        })
    },
    a.fn.live = function(v, w) {
        return a(document.body).delegate(this.selector, v, w),
        this
    },
    a.fn.die = function(v, w) {
        return a(document.body).undelegate(this.selector, v, w),
        this
    },
    a.fn.on = function(w, x, v) {
        return ! x || a.isFunction(x) ? this.bind(w, x || v) : this.delegate(x, w, v)
    },
    a.fn.off = function(w, x, v) {
        return ! x || a.isFunction(x) ? this.unbind(w, x || v) : this.undelegate(x, w, v)
    },
    a.fn.trigger = function(v, w) {
        if (typeof v == "string" || a.isPlainObject(v)) {
            v = a.Event(v)
        }
        return m(v),
        v.data = w,
        this.each(function() {
            "dispatchEvent" in this && this.dispatchEvent(v)
        })
    },
    a.fn.triggerHandler = function(x, y) {
        var v, w;
        return this.each(function(A, z) {
            v = i(typeof x == "string" ? a.Event(x) : x),
            v.data = y,
            v.target = z,
            a.each(u(z, x.type || x),
            function(B, C) {
    ...
    ["focus", "blur"].forEach(function(v) {
        a.fn[v] = function(w) {
            return w ? this.bind(v, w) : this.each(function() {
                try {
                    this[v]()
                } catch(x) {}
            }),
    ...
    a.Event = function(w, x) {
        typeof w != "string" && (x = w, w = x.type);
        var y = document.createEvent(e[w] || "Events"),
        z = !0;
        if (x) {
            for (var v in x) {
                v == "bubbles" ? z = !!x[v] : y[v] = x[v]
            }
        }
        return y.initEvent(w, z, !0, null, null, null, null, null, null, null, null, null, null, null, null),
        y.isDefaultPrevented = function() {
...
function(a, b) {
    function u(v) {
        return i(v.replace(/([a-z])([A-Z])/, "$1-$2"))
    }
    function i(v) {
    ...
    a.each(r,
    function(v, w) {
        if (g.style[v + "TransitionProperty"] !== b) {
            return c = "-" + i(v) + "-",
            d = w,
            !1
        }
    ...
    o = c + "transform",
    n[h = c + "transition-property"] = n[l = c + "transition-duration"] = n[p = c + "transition-timing-function"] = n[q = c + "animation-name"] = n[s = c + "animation-duration"] = n[f = c + "animation-timing-function"] = "",
    a.fx = {
        off: d === b && g.style.transitionProperty === b,
        speeds: {
            _default: 400,
            fast: 200,
            slow: 600
    ...
    a.fn.animate = function(x, y, v, w) {
        return a.isPlainObject(y) && (v = y.easing, w = y.complete, y = y.duration),
        y && (y = (typeof y == "number" ? y: a.fx.speeds[y] || a.fx.speeds._default) / 1000),
        this.anim(x, y, v, w)
    },
    a.fn.anim = function(D, E, F, v) {
        var w, x = {},
        y, z = "",
        A = this,
        B, C = a.fx.transitionEnd;
        E === b && (E = 0.4),
        a.fx.off && (E = 0);
        if (typeof D == "string") {
            x[q] = D,
            ...
            y = [];
            for (w in D) {
                k.test(w) ? z += w + "(" + D[w] + ") ": (x[w] = D[w], y.push(u(w)))
            }
            z && (x[o] = z, y.push(o)),
            E > 0 && typeof D == "object" && (x[h] = y.join(", "), x[l] = E + "s", x[p] = F || "linear")
        }
        return B = function(G) {
